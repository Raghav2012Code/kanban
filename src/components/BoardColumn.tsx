...
export function AddColumn({ value, inputRef, onChange, onSave, onCancel }: AddColumnProps): JSX.Element {
  ...
  return <div className="w-full min-w-0 shrink-0 rounded-xl border border-zinc-800 bg-[#09090b] p-3 lg:w-0 lg:min-w-0 lg:flex-1"><div className="flex gap-2"><input ref={inputRef} value={value} onChange={(event) => onChange(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') onSave(); if (event.key === 'Escape') onCancel(); }} placeholder="Column name" aria-label="New column name" className="min-w-0 flex-1 rounded border border-zinc-800 bg-zinc-900 px-2.5 py-2 text-sm text-zinc-200 outline-none focus:border-zinc-500" /><button type="button" onClick={onSave} aria-label="Save column" className="rounded bg-zinc-100 px-2 text-zinc-950 hover:bg-white"><Check className="h-4 w-4" /></button><button type="button" onClick={onCancel} aria-label="Cancel adding column" className="rounded border border-zinc-800 px-2 text-zinc-400 hover:text-zinc-200"><X className="h-4 w-4" /></button></div></div>;
}
