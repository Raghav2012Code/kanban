...
export default function KanbanBoard(): JSX.Element {
  ...
  const handleDragOver = (event: DragEvent<HTMLElement>, columnId: string, cardId?: string) => { event.preventDefault(); event.dataTransfer.dropEffect = 'move'; if (!cardId) { setDropTarget({ columnId }); return; } const rect = event.currentTarget.getBoundingClientRect(); setDropTarget({ columnId, cardId, insertAfter: event.clientY >= rect.top + rect.height / 2 }); };
  ...
  const handleDrop = (event: DragEvent<HTMLElement>, columnId: string, cardId?: string) => { event.preventDefault(); const id = event.dataTransfer.getData('text/plain'); if (id && board.cards[id]) updateBoard((current) => moveCard(current, id, columnId, cardId, dropTarget?.insertAfter ?? false)); clearDrag(); };
